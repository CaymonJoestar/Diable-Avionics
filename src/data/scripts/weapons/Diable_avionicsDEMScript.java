 ...
 *
 */
public class Diable_avionicsDEMScript extends BaseEveryFrameCombatPlugin implements MissileAIPlugin {
    ...
    protected com.fs.starfarer.api.impl.combat.dem.DEMScript.State state = com.fs.starfarer.api.impl.combat.dem.DEMScript.State.WAIT;
    ...
    protected com.fs.starfarer.api.impl.combat.dem.DEMScript.ShapedExplosionParams p;

    public  Diable_avionicsDEMScript (MissileAPI missile, ShipAPI ship, WeaponAPI weapon) {
        //初始化DEM脚本类并且从json里读取相关参数
        ...
        try {
            if (json.optBoolean("withShapedExplosion")) {
                p = new com.fs.starfarer.api.impl.combat.dem.DEMScript.ShapedExplosionParams();
                p.load(json);;
            }
        } catch (Exception e) {
    ...
    public void advance(float amount, List<InputEventAPI> events) {
        ...
        boolean doCleanup = state == com.fs.starfarer.api.impl.combat.dem.DEMScript.State.DONE ||
                (!bombPumped || state.ordinal() < com.fs.starfarer.api.impl.combat.dem.DEMScript.State.FIRE.ordinal()) &&
        ...
        if (state == com.fs.starfarer.api.impl.combat.dem.DEMScript.State.WAIT && missile.isArmed() && !missile.isFizzling() && !missile.isFading()) {
            ...
            if (target != null && elapsedWaiting >= minDelayBeforeTriggering) {
                ...
                if (dist < triggerDistance) {
                    ...
                    state = com.fs.starfarer.api.impl.combat.dem.DEMScript.State.TURN_TO_TARGET;
                    fireTarget = target;
        ...
        } else if (state == com.fs.starfarer.api.impl.combat.dem.DEMScript.State.TURN_TO_TARGET) {
            ...
            if (Misc.isInArc(missile.getFacing(), targetingLaserArc, angle)) {
                ...
                state = com.fs.starfarer.api.impl.combat.dem.DEMScript.State.SIGNAL;
            }
        } else if (state == com.fs.starfarer.api.impl.combat.dem.DEMScript.State.SIGNAL) {
            ...
            if (elapsedTargeting >= targetingTime) {
                state = com.fs.starfarer.api.impl.combat.dem.DEMScript.State.FIRE;
        ...
        } else if (state == com.fs.starfarer.api.impl.combat.dem.DEMScript.State.FIRE) {
...
//				p.shapedExplosionMaxParticleDur /= speedMult;
            ...
            if (elapsedFiring >= firingTime) {
                ...
                state = com.fs.starfarer.api.impl.combat.dem.DEMScript.State.DONE;
    ...
    protected void doMissileControl(float amount) {
        if (state == com.fs.starfarer.api.impl.combat.dem.DEMScript.State.TURN_TO_TARGET || state == com.fs.starfarer.api.impl.combat.dem.DEMScript.State.SIGNAL ||
                (state == com.fs.starfarer.api.impl.combat.dem.DEMScript.State.FIRE && !bombPumped && !fadeOutEngineWhenFiring)) {
...
//			boolean turningTowardsDesiredFacing = Math.signum(turnDir1) == Math.signum(missile.getAngularVelocity());
    ...
    public void spawnShapedExplosion(Vector2f loc, float angle, com.fs.starfarer.api.impl.combat.dem.DEMScript.ShapedExplosionParams p) {
